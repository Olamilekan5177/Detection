    ...
    def process_region(self, region_id=None, days_back=7):
        ...
        try:
            ...
            try:
                ...
                # Note: Full Sentinel-1 download requires complex authentication
                # Instead, we use the fallback method which generates synthetic Sentinel-1 data
                logger.info("Using fallback Sentinel-1 synthetic data generator...")
                logger.info("(Full tile downloads require SciHub credentials and OAuth2)")
                
                # Extract metadata from first tile for logging
                first_tile = tiles[0]
                tile_info = f"ID: {first_tile.get('id', 'N/A')}, "
                if 'acquisition_date' in first_tile:
                    tile_info += f"Date: {first_tile['acquisition_date']}"
                logger.info(f"Using metadata from: {tile_info}")
