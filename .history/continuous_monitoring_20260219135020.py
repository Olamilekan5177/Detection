...
def run_monitoring_iteration(regions: dict, model_path: str):
    ...
    logger.info("=" * 70)
    logger.info(f"MONITORING ITERATION: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("=" * 70)
    ...
    logger.info(f"✓ Sentinel Hub credentials loaded: {config.client_id[:10]}***")
    ...
    for region_name, config_data in regions.items():
        ...
        logger.info(f"   Bbox: {config_data['bbox']}")
        ...
        try:
            ...
            if results and results.get("status") == "success":
                ...
                if detections > 0:
                    logger.warning(f"   🚨 OIL SPILL DETECTED IN {region_name}!")
            ...
            logger.error(f"   ✗ Error processing {region_name}: {e}")
            results_summary["errors"] += 1
    
    # Summary
    logger.info("\n" + "=" * 70)
    logger.info("ITERATION SUMMARY")
    logger.info(f"  Regions processed: {results_summary['processed']}")
    logger.info(f"  Total detections: {results_summary['detections']}")
...
def start_continuous_monitoring(interval_hours: int = 24, max_iterations: int = None):
    ...
    logger.info(f"   Active regions: {', '.join(enabled_regions)}")
    logger.info("")
