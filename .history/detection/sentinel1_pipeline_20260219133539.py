...
class Sentinel1QueryEngine:
    ...
    def search_tiles(
        ...
        try:
            # Build OData filter query for Sentinel Hub Catalog API
            filters = [
                "Collection/Name eq 'SENTINEL-1'",
                "Attributes/OData.CSC.StringAttribute/any(d:d/Name eq 'productType' and d/OData.CSC.StringAttribute/Value eq 'GRD')",
            ]
            
            # Add date filter
            date_filter = f"ContentDate/Start gt {start_date.isoformat()}Z and ContentDate/Start lt {end_date.isoformat()}Z"
            filters.append(date_filter)
            
            # Add pass direction if specified
            if pass_direction:
                filters.append(
                    f"Attributes/OData.CSC.StringAttribute/any(d:d/Name eq 'orbitDirection' and d/OData.CSC.StringAttribute/Value eq '{pass_direction}')"
                )
            
            filter_str = " and ".join(filters)
            
            # Build spatial filter for bbox
            # Using geometry intersection: bbox(min_lon, min_lat, max_lon, max_lat)
            spatial_filter = f"geo.intersects(Geometry, geography'POLYGON(({bbox[0]} {bbox[1]}, {bbox[2]} {bbox[1]}, {bbox[2]} {bbox[3]}, {bbox[0]} {bbox[3]}, {bbox[0]} {bbox[1]}))')"
