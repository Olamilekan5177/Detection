...
class Sentinel1QueryEngine:
    ...
    def search_tiles(
        ...
        try:
            ...
            # Build simplified OData filter query for Sentinel Hub Catalog API
            # Using simpler, more reliable syntax
            filters = [
                "Collection/Name eq 'SENTINEL-1'",
                "OData.CSC.Attributes/any(a:a/Name eq 'productType' and a/OData.CSC.StringAttribute/Value eq 'GRD')",
            ]
            
            # Add date filter with proper ISO format
            date_filter = f"ContentDate/Start ge {start_str}Z and ContentDate/Start le {end_str}Z"
            filters.append(date_filter)
            
            # Add pass direction if specified
            if pass_direction:
                filters.append(
                    f"OData.CSC.Attributes/any(a:a/Name eq 'orbitDirection' and a/OData.CSC.StringAttribute/Value eq '{pass_direction}')"
                )
            
            filter_str = " and ".join(filters)
            
            # Build spatial filter for bbox using simple bbox syntax
            # Sentinel Hub Catalog uses: geo.intersects(Geometry, geography'SRID=4326;POLYGON((lon1 lat1, lon2 lat1, lon2 lat2, lon1 lat2, lon1 lat1))')
            spatial_filter = f"geo.intersects(Geometry, geography'SRID=4326;POLYGON(({bbox[0]} {bbox[1]}, {bbox[2]} {bbox[1]}, {bbox[2]} {bbox[3]}, {bbox[0]} {bbox[3]}, {bbox[0]} {bbox[1]}))')"
            
            query_url = f"{self.catalog_url}/Products"
            query_params = {
                "$filter": f"{filter_str} and {spatial_filter}",
                "$top": limit,
