...
def monitoring_status(request):
    """
    Real-time monitoring status view
    """
    
    context = {
        'page_title': 'Monitoring Status',
    ...
    try:
        with open('monitoring.log', 'r') as f:
            ...
            context['monitoring_log'] = ''.join(log_lines)
    except FileNotFoundError:
        context['monitoring_log'] = 'No monitoring logs available'
    
    # Get pipeline state
    try:
        with open('pipeline_state.json', 'r') as f:
            context['pipeline_state'] = json.load(f)
    except FileNotFoundError:
        context['pipeline_state'] = {'status': 'not_running'}
    
    # Monitoring regions status
    context['regions_status'] = get_regions_monitoring_status()
    
    return render(request, 'dashboard/monitoring_status.html', context)
...
def get_monitoring_status():
    ...
    try:
        ...
        return {
            'running': False,
            'error': 'No monitoring logs found'
        }
